...
export default function MailLogs(props: { onClose: Function; currentMember: defaultMemberSelectOutput }) {
  ...
  return (
    <div className="divide-y divide-gray-200 lg:col-span-9 max-w-full">
      <div className="py-6 px-4 sm:p-6 lg:pb-8">
        <div>
          <h2 className="text-lg font-medium leading-6 text-gray-900">{t('EmailHistory')}</h2>
          <p className="mt-1 text-sm text-gray-500">{t('Displaying_the_last_90_days')}</p>
        </div>
        <div className="mt-6">
                ...
                <table className="min-w-full divide-y divide-gray-200 table-fixed">
                  <thead className="bg-gray-50">
                    <tr>
                      <th
                        scope="col"
                        className={`px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider ${hasStatusMessage ? 'w-1/3' : 'w-1/2'}`}
                      >
                        {t('Subject')}
                      </th>
                      <th
                        scope="col"
                        className="w-1/12 px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
                      >
                        {t('Delivery_Status')}
                      </th>
                      {hasStatusMessage && (
                      ...
                      <th
                        scope="col"
                        className="w-1/6 px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
                      >
                        {t('Timestamp')}
                      </th>
                    </tr>
                  ...
                  <tbody className="divide-y divide-gray-200 bg-white">
                    {mailHistoryData?.map((mail, index) => (
                      <tr key={index}>
                        <td
                          className="px-4 py-2 text-sm text-gray-500 break-words whitespace-normal max-w-xs"
                        ...
                        {hasStatusMessage && (
                          <td
                            className="px-4 py-2 text-sm text-gray-500 break-all whitespace-normal max-w-lg"
