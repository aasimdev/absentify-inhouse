      ...
      {groups.map((group) => (
        <div key={group.id} className="flex items-center ml-3 py-0.5">     
          <input
            type="checkbox"
            id={group.id}
            className="focus:ring-teams_brand_500 h-4 w-4 text-teams_brand_600 border-gray-300 rounded"
            checked={selectedGroups.includes(group.id)}
            onChange={(event) => handleGroupChange(event, group)}
          />
          <label htmlFor={group.id} className="ml-2 block text-sm font-medium text-gray-700">
