...
export default function UpgradeModalCalendarSync(props: { open: boolean; onClose: Function }) {
  const { t } = useTranslation('upgrade');
  const changeCalendarSyncSubscription = api.subscription.change_calendar_sync_subscription.useMutation();
  ...
  useEffect(() => {
    ...
    setValue('quantity', subscription.addons.calendar_sync - 1);
  }, [subscription]);
  let oldSubscription = '';
  ...
  const { register, handleSubmit, setValue, watch, getValues } =
    useForm<RouterInputs['subscription']['change_calendar_sync_subscription']>();

  const onSubmit: SubmitHandler<RouterInputs['subscription']['change_calendar_sync_subscription']> = async (
    data: RouterInputs['subscription']['change_calendar_sync_subscription']
  ) => {
    if (!current_member || !workspace) return;

    if (subscription.addons.calendar_sync == data.quantity) return;
    setIsLoading(true);
    await changeCalendarSyncSubscription.mutateAsync(
  ...
  useEffect(() => {
    ...
    if (subscription.billing_cycle_interval == 'month') {
      let part = getPriceByName(paddlePrices, false, 'SHARED_OUTLOOK_CALENDAR_SYNC_ADDON');
      priceLabel =
        currencies[(paddlePrices.data.currencyCode as 'USD' | 'EUR') ?? 'USD']?.symbol +
        (Number(part?.totals.subtotal) / 100) * getValues('quantity') +
        ' / ' +
        t('Month');
    } else {
      let part = getPriceByName(paddlePrices, true, 'SHARED_OUTLOOK_CALENDAR_SYNC_ADDON');
                    ...
                    <div className="mt-6 grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-6">
                      <div className="sm:col-span-5">
                        <label htmlFor="username" className="block text-sm font-medium text-gray-700">
                          {t('Quantity_description_calendarSync')}
                        </label>
