      ...
      <table className="min-w-full divide-y divide-gray-200 border z-0">
        ...
        <tbody className="divide-y divide-gray-200 bg-white  ">
          {publicHolidayDays
            ...
            .map((holiday) => (
              <tr key={holiday.id}>
                <td colSpan={2} className="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-900 ">
                  <div className="w-32 truncate">{holiday.name} </div>
                  <div>{format(dateToIsoDate(holiday.date), current_member?.date_format)}</div>
                </td>
          ...
          <tr>
            ...
            >
              <div className="flex">
                ...
                <span className="ml-2"> {t('Add_public_holiday_day')}</span>
              </div>
            </td>
          </tr>
        </tbody>
